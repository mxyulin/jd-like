          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr of spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: spuSaleVal.isChecked == 1 }"
                  v-for="spuSaleVal of spuSaleAttr.spuSaleAttrValueList"
                  :key="spuSaleVal.id"
                  @click="checkSaleAttrVal(spuSaleAttr.id, spuSaleVal.id)"
                >
                  {{ spuSaleVal.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
...
export default {
  ...
  methods: {
    // 选中某个售卖属性值时，更新售卖属性值的状态 isChecked
    checkSaleAttrVal(saleAttrId, saleValId) {
      this.$store.commit("Detail/CHANGEISCHECKED", { saleAttrId, saleValId });
    },
    // *输入校验：只准输入正整数
    changekuNum(e) {
      // 非法和空串重置 1
      let val = e.target.value;
      if (isNaN(val) || val == "" || val == 0) return (this.skuNum = 1);
      // 负数取正整
    ...
    addToCart() {
      this.$store
        ...
        .then((res) => {
          ...
          // 加入购物车成功的展示信息（存储在sessionStorage）
          const { skuDefaultImg, skuName } = this.skuInfo;
